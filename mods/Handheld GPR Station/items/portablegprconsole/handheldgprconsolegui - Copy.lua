...
function init()
  debugMode = false
  GPRregion = 5
--EDIT_HERE  GPRrect = {entity.position()[1]-GPRregion, entity.position()[2]-GPRregion, entity.position()[1]+GPRregion, entity.position()[2]+GPRregion}
  -- id, position
  stationInfo = { player.id(), world.entityPosition(player.id()) } --EDIT_HERE
  self.senderEntityPosition = stationInfo[1] --EDIT_HERE
	self.scanMatrix = {}
	self.bounds = {-195, -105, 195, 105}
  retryScan = false
  --EDIT_HERE script.setUpdateDelta(15)
	liquidIndex = {
  ...
  --EDIT_HERE ^^^^^ taken from gprconsoleobject
...
--EDIT_HERE vvvv gbrconsoleobject
...
function loadWithinBounds(senderEntityPosition, bounds)
	...
	return realBounds--world.loadRegion(realBounds)
end
